from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QPushButton, QVBoxLayout, QHBoxLayout, QFrame, QMainWindow
from PyQt5.QtGui import QFont, QPixmap, QIcon, QRegularExpressionValidator
from PyQt5.QtCore import Qt, QTimer, QRegularExpression, QSize, QEvent
import json
import traceback
import sys
import os

# --- Mock API Service and FireDetectionApp for standalone execution ---
# REMOVE these mock classes if you have the actual files in your project
class EnvConfig:
    def __init__(self):
...
class FireDetectionApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Mock Fire Detection Dashboard")
        self.setGeometry(100, 100, 800, 600)
        central_widget = QWidget()
        self.setCentralWidget(central_widget)
        layout = QVBoxLayout(central_widget)
        layout.addWidget(QLabel("Welcome to the Fire Detection Dashboard! (Mock)"))
        print("Mock FireDetectionApp opened.")
# --- END Mock Classes ---
...
class FireDetectionUI(QWidget):
    def __init__(self):
        super().__init__()
        # Basic window setup
        self.setWindowTitle("Reliable Detection. Unwavering Security")
        ...
        # Removed explicit resizeEvent call here. It will be handled via QTimer.singleShot in __main__
    ...
    def open_main_window(self):
        ...
        try:
            self.parking_window = FireDetectionApp()
            if self.isMaximized():
...
if __name__ == "__main__":
    ...
    window.show()
    # Force a resize event and then polish the style after the window is shown.
    # This combination is often effective for ensuring initial rendering.
    QTimer.singleShot(0, lambda: window.setGeometry(window.geometry())) # Force a resize event
    QTimer.singleShot(0, lambda: QApplication.instance().style().polish(window)) # Then polish the style
    sys.exit(app.exec_())
